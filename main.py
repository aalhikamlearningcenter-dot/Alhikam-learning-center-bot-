# ============================================================
# ALHIKAM LEARNING CENTER BOT V2
#
# FLOW:
# Flutterwave Payment
#        ↓
...
#
# SECURE PAYMENT + REFERRAL + COMMISSION + WITHDRAWAL
# ============================================================
...
def create_payment():
    ...
    payment = {
        ...
        # IMPORTANT:
        # database.py stores this as `status`
        "status":
            "pending",
    ...
    try:
        ...
        logger.info(
            "Flutterwave checkout status=%s tx_ref=%s",
...
def _payment_from_token(
    ...
    if row:
        ...
        # CRITICAL FIX
        #
        # database.py stores payment state in `status`.
        #
        # OLD CODE incorrectly used:
        # data.get("payment_status")
        #
        # That caused successful payments to appear pending.
        # ====================================================
        ...
        # ====================================================
        # RESTORE TELEGRAM AUTH AFTER RAILWAY RESTART
        #
        # database.py persists telegram_id and username.
        # It does NOT persist telegram_name.
        # ====================================================
...
def find_flutterwave_transaction_by_tx_ref(
    ...
    try:
        ...
        if not isinstance(
            ...
            logger.warning(
                "Unexpected Flutterwave transaction list."
            )
        ...
        for transaction in data:
            ...
            if (
                ...
                logger.info(
                    "Flutterwave transaction found tx_ref=%s id=%s",
                    tx_ref,
                    transaction.get("id"),
                )
        ...
        logger.exception(
            "Error finding Flutterwave transaction tx_ref=%s",
            tx_ref,
        )
...
def verify_flutterwave_transaction(
    ...
    try:
        ...
        logger.info(
            "Flutterwave verification HTTP status=%s transaction_id=%s",
            response.status_code,
            transaction_id,
        )
        ...
        if response.status_code != 200:
            ...
            logger.error(
                "Flutterwave verification failed body=%s",
                response.text[:1000],
            )
        ...
        if (
            ...
            logger.warning(
                "Flutterwave verification returned non-success response=%s",
                result,
            )
...
def _verify_and_finalize_payment(
    ...
    if not transaction_id:
        ...
        logger.info(
            "No transaction_id supplied. Searching by tx_ref=%s",
            expected_tx_ref,
        )
        ...
        if not transaction_id:
            ...
            logger.warning(
                "Transaction not found by tx_ref=%s",
                expected_tx_ref,
            )
    ...
    if not verified:
        ...
        logger.warning(
            "Could not verify transaction tx_ref=%s id=%s",
            expected_tx_ref,
            transaction_id,
        )
    ...
    if verified_status != "successful":
        ...
        logger.warning(
            "Transaction is not successful tx_ref=%s status=%s",
            verified_tx_ref,
            verified_status,
        )
    ...
    try:
        ...
        logger.exception(
            "Invalid payment amount tx_ref=%s",
            verified_tx_ref,
        )
    ...
    if promoter:
        ...
        if promoter_status != "active":
            ...
            referral_code
            and promoter_referral_code.lower()
            != referral_code.lower()
        ):

            logger.error(
                "Promoter referral mismatch payment=%s promoter=%s",
                referral_code,
    ...
    try:
        ...
        logger.exception(
            "Failed updating payment status tx_ref=%s",
            verified_tx_ref,
        )
    ...
    try:
        ...
        logger.exception(
            "Failed saving verified payment tx_ref=%s",
            verified_tx_ref,
        )
    ...
    logger.info(
        "ALHIKAM PAYMENT VERIFIED SUCCESSFULLY tx_ref=%s transaction_id=%s",
        verified_tx_ref,
        transaction_id,
    )
...
def payment_complete(
    ...
    if (
        ...
        logger.error(
            "Redirect tx_ref mismatch expected=%s received=%s",
            expected_tx_ref,
            returned_tx_ref,
        )
    ...
    #
    # Retry a few times because Flutterwave may take
    # a short moment before the transaction becomes
    ...
    if (
        ...
        logger.info(
            "Payment verified. Redirecting to Telegram login token=%s",
            payment_token,
        )
...
connect the Telegram account you will use
to receive your ALHIKAM class invite.
...
def registration_form(
    ...
    if (
        ...
        return (
            "Invalid registration request. Please refresh and try again.",
            400,
        )
    ...
    # IMPORTANT:
    # Keep payment status successful.
    save_payment({
...
def flutterwave_webhook():
    ...
    # ========================================================
    # VERIFY CALLBACK WITH FLUTTERWAVE
    # ========================================================
    ...
    if (
        ...
        logger.info(
            "Webhook successfully finalized tx_ref=%s",
            verified_tx_ref,
        )
...
def main():
    ...
    print(
        "Secure Payment Verification: ENABLED"
    )
