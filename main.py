...
# ============================================================
# TEMPORARY STORAGE
# NOTE: Railway restarts clear this memory.
    ...
    if row:
        ...
        if data.get("telegram_id"):
            data["telegram_auth"] = {
                ...
                "first_name": (
                    data.get("telegram_name") or ""
                ).split(" ")[0],
                "last_name": " ".join(
                    (data.get("telegram_name") or "").split(" ")[1:]
                ),
            }
...
<p><strong>⚠️ Do not enter your Telegram ID manually.</strong></p>

<p>Click the Telegram button below to connect your account.</p>
</div>
</body>
</html>
"""
...
def verify_telegram_login(data):
    ...
    try:
        if abs(int(time.time()) - int(auth_date)) > 3600:
            return False
    except Exception:
        return False
    ...
    save_payment(
        {
            ...
            "payment_plan": payment.get(
                "plan_name", ""
            ),
            "amount": payment.get("amount", 0),
            "payment_status": "Successful",
            "referral_code": payment.get(
                "referral_code", ""
            ),
            "promoter_id": payment.get(
                "promoter_id"
            ),
            "promoter_name": payment.get(
                "promoter_name", ""
            ),
            "commission": payment.get(
                "commission", 0
            ),
            "telegram_id": data["id"],
            "telegram_username": data["username"],
...
def registration_csrf(payment_token):
    ...
    save_payment(
        {
            ...
            "commission": _commission_for_amount(
                payment.get("amount", 0)
            ),
            "telegram_id": telegram_auth[
                "telegram_id"
            ],
            "telegram_username": telegram_auth.get(
                "telegram_username",
...
def flutterwave_webhook():
    ...
    if (
        ...
        return jsonify(
            {
                "status": "error",
                "message": "Transaction verification mismatch",
            }
        ), 400
    ...
    promoter = (
        get_promoter_by_id(promoter_id)
        if promoter_id
        else None
    )
...
def flutterwave_transfer_callback():
    try:
        ...
        callback_reference = (
            data.get("reference")
            or data.get("transfer_reference")
            or request.form.get("reference")
            or request.form.get("transfer_reference")
        ...
        if (
            callback_reference
            and local_reference
            and callback_reference != local_reference
        ):
            logger.error(
        ...
        if transfer_id:
            verified = get_flutterwave_transfer_status(
                transfer_id
            )
        else:
            verified = get_flutterwave_transfer_status_by_reference(
                local_reference
            )
        ...
        if (
            ...
            and verified_reference != local_reference
        ):
            logger.error(
